...
namespace RestSharpServices
{
    public class GitHubApiClient
    {
        ...
        {
            var reqest = new RestRequest($"{repo}/issues");
            reqest.AddJsonBody(new { title, body });
            var response = client.Execute(reqest, Method.Post);

            return response != null ? JsonSerializer.Deserialize<Issue>(response.Content) : null;
        }
        ...
        {
            var reqest = new RestRequest($"{repo}/issues/{issueNumber}/comments");
            reqest.AddJsonBody(new { body });
            var response = client.Execute(reqest, Method.Post);

            return response != null ? JsonSerializer.Deserialize<Comment>(response.Content) : null;
        }
